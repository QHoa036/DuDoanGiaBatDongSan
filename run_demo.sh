...
if [ ! -d "Demo" ]; then
    log "ERROR" "Không tìm thấy thư mục Demo. Vui lòng kiểm tra cấu trúc dự án!"
    exit 1
fi
...
EOF
    ...
    $PYTHON_PATH .temp_ngrok_setup.py --streamlit_path "$STREAMLIT_PATH" --app_path "Demo/main.py" --ngrok_token "$NGROK_TOKEN"
...
if [[ $use_ngrok == "y" || $use_ngrok == "Y" ]]; then
    ...
    $STREAMLIT_PATH run Demo/main.py
fi
